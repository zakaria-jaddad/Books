...
HEY :) This was my notes while reading the book.

There could be some errors, topics i didn't understand well any pull requestion would be appreciated.
...
- [what is Bitwise manipulation?](#what-is-bitwise-manipulation)
...
- [Chapter 3 - Control Flow](#chapter-3-control-flow)
  ...
  - [3.8 Got and labels](#38-got-and-labels)
...
- [Chapter 5 - Pointers and Arrays](#chapter-5-pointers-and-arrays)
  ...
  - [5.10 Command-line Arguments](#510-command-line-arguments)
    ...
    - [Exercise 5-11 and 5-12.](#exercise-5-11-and-5-12)
...
- [Chapter 6 - Structures](#chapter-6-structures)
  ...
  - [6.2 Structures and Functions](#62-structures-and-functions)
    - [Pass components separately & Pass an entire structure.](#pass-components-separately-pass-an-entire-structure)
    - [Pass a pointer to it.](#pass-a-pointer-to-it)
  ...
  - [6.6 Table Lookup](#66-table-lookup)
    - [Exercise 6-5 6-6.](#exercise-6-5-6-6)
...
Execution time slightlly difference
...
Execution time is slightlly difference

<!-- [Code Solution]() -->

The system call `stat` takes a filename and returns all of the inforamation in the inode for that file, or `-1` if there is and error,
That is:
...
So `stat` fills the structure `stbuf` with the indoe information for the file name.

The structure describing the value returned by `stat`.

`stat` typically is in `<sys/stat.h>` and looks like this:
...
The types `dev_t` and `ino_t` are defined in `<sys/types.h>`

The `st_mode` contains a set of flags describing the file.

The flag definitions are also included in `<sys/types.h>` we only need the part that deals with file types.
...
The blocks are kept in order of increasing storage address, and the
last block `(highest address)` points to the first.

![Image of How malloc works](./Media/malloc-routine.png)

When a request is made, the free list is scanned until a big-enough block is found.

This algorithm is called `first fit` by contrast with `best fit` which looks for the smallest block
...
- If the block is too big, it is split, and the proper amount is
  returned to the user while the residue remains on the free list.

- If no big-enough block is found,
...
If the block being freed is adjacent to a free block on either side, it is coalesced aka colabsed with it
...
Although machines vary, for each machine there is a most restrictive type: if the most restrictive type can be stored at a
...
![Malloc returned value](./Media/malloc-returned-value.png)
